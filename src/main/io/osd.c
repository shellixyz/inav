...
/*
static void osdDisplayAdjustableIntegerValue(uint8_t elemPosX, uint8_t elemPosY, const char *str, const uint8_t value_offset, const int16_t value, adjustmentFunction_e adjFunc) {
    displayWrite(osdDisplayPort, elemPosX, elemPosY, str);
    char buff[8];
...
static bool osdDrawSingleElement(uint8_t item)
{
    ...
    case OSD_FW_MIN_THROTTLE_DOWN_PITCH_ANGLE:
        osdDisplayAdjustableDecimalValue(elemPosX, elemPosY, "0TP", 0, (float)mixerConfig()->fwMinThrottleDownPitchAngle / 10, 4, 0, ADJUSTMENT_FW_MIN_THROTTLE_DOWN_PITCH_ANGLE);
        return true;
    ...
    case OSD_EFFICIENCY_MAH_PER_KM:
        {
            ...
            if (STATE(GPS_FIX) && gpsSol.numSat >= 6 && gpsSol.groundSpeed >= 200) {
